...
class ProtagonistaDisfrazado:
    ...
    def recuperar_casas_DFS_recursivo(
        self, casa_actual: Casa, distancia_actual: int = 0
    ) -> list:
    ...
    def recuperar_casas_DFS_iterativo(self, casa_inicial: Casa) -> list:
        ...
        def distancia(casa: Casa) -> int:
            """retorna la distancia de la casa actual a la casa inicial"""
        ...
        while len(stack) > 0:
            ...
            if casa in visitados:
                ...
                visitados.append({"casa": casa, "distancia": distancia(casa)})
    ...
    def filtrar_casas(self, listado_casas: list) -> list:
        ...
        for dic_casa in listado_casas:
            if dic_casa["casa"].decoracion >= p.EXPECTATIVAS_DECORACION:
                puntaje = (
                    p.PONDERADOR_DECORACION * dic_casa["casa"].decoracion
